...
Cac giai doan:
  --phase develop: Huan luyen tren Train/Val
  --phase lock: Tao protocol_lock.json va kiem tra preflight check
  --phase final-test: Danh gia 1-pass tren TestLoader voi lock_token
...
def run_canonical_multi_seed(
    ...
    print("=" * 80)
    print(f"CANONICAL DETECTION PROTOCOL | Phase: {phase.upper()}")
    ...
    lock_token = None
    if phase in ("lock", "final-test", "all"):
        print("\n>>> BAT DAU PHASE 2: PROTOCOL LOCK & PREFLIGHT CHECK <<<")
        # Sinh protocol_lock.json
        generate_protocol_lock(
            data_root=data_root,
        ...
        # Chay global preflight check de cap token
        passed, lock_token = global_preflight_check(lock_path=lock_path, data_root=data_root)
        if not passed:
